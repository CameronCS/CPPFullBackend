...
int main() {
	...
	namespace DI = SystemFramework::DependencyInjection;
	DI::DependencyContainer container;

	container.Register<DataService::Interface::IProductRepository>(
		[&connStr, &dataLogger](DI::ScopedContainer&) -> void* {
			return new DataService::ProductRepository(connStr, &dataLogger);
		}
	);

	container.Register<BusinessService::Interface::IProductService>(
		[&businessLogger](DI::ScopedContainer& scope) -> void* {
			auto* repo = scope.Resolve<DataService::Interface::IProductRepository>();
			return new BusinessService::ProductService(repo, &businessLogger);
		}
	);

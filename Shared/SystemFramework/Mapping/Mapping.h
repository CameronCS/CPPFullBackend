...
namespace SystemFramework::Mapping {
	...
	public:
		template<typename TSource, typename TDest> void Register(std::function<TDest(const TSource&)> mappingFunc) {
			...
			_mappings[key] = 
				[mappingFunc](void* src) -> void*
				{
					return new TDest(mappingFunc(*static_cast<TSource*>(src)));
				};
		}

		template<typename TSource, typename TDest> TDest Map(const TSource& source) {
			std::pair<std::type_index, std::type_index> key = std::make_pair(std::type_index(typeid(TSource)), std::type_index(typeid(TDest)));
			std::unordered_map<std::pair<std::type_index, std::type_index>, std::function<void* (void*)>>::iterator it = _mappings.find(key);
			if (it == _mappings.end()) {
	...
	private:
		...
		std::unordered_map<std::pair<std::type_index, std::type_index>, std::function<void* (void*)>, PairHash> _mappings;
	};
}
